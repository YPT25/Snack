...
public class PlayerScriptA : Damageable<DamageableSO>
{
    ...
    void Update()
    {
        ...
        if (isLocalPlayer) // !isLocalPlayer ではなく isLocalPlayer
        {
            ...
            //攻撃テスト
            if (Input.GetMouseButtonUp(0)) //短距離
                TestAttack(3f);
            else if (Input.GetMouseButtonUp(1)) //長距離
                TestAttack(10f);
        }
    ...
    //攻撃テスト
    private void TestAttack(float range)
    {
        Collider[] hits = Physics.OverlapSphere(transform.position, range);
        foreach (Collider hit in hits)
        {
            if (hit.TryGetComponent(out EnemyScript enemy))
                enemy.TakeDamage(_damageableData._atk, gameObject);
        }
    }
}